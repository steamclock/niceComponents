//
//  TextTheme.swift
//  NiceComponents
//
//  Created by Brendan on 2024-01-29.
//  Copyright © 2024 Steamclock Software. All rights reserved.
//

import SwiftUI

/// Styling settings for a text element.
public protocol TextTheme {
    /// The color your text should be.
    var color: Color { get }

    /// The name of the font the text should use.
    var font: String? { get }

    /// Font weight to use.
    var weight: Font.Weight? { get }

    /// The size of the text to use.
    var size: CGFloat { get }

    /// The space between the characters.
    var tracking: CGFloat { get }

    /// The maximum dynamic type size text should be scaled to.
    var dynamicTypeMaxSize: DynamicTypeSize? { get }

    /// The number of lines to limit the text to.
    var lineLimit: Int? { get }

    /// The space between lines of the text.
    var lineSpacing: CGFloat { get }

//    func with(color: Color?, font: String?, weight: Font.Weight?, size: CGFloat?, tracking: CGFloat?, dynamicTypeMaxSize: DynamicTypeSize?, lineLimit: Int?, lineSpacing: CGFloat?) -> TextTheme
}
